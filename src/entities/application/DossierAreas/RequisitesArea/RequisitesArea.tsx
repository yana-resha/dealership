...
import { Requisite } from 'entities/application/DossierAreas/Requisite/Requisite'
  ...
  return (
    <Box className={classes.requisitesBlock}>
      ...
      {additionalEquipment.length > 0 && (
        <Box className={classes.requisiteContainer}>
          ...
          {additionalEquipment.map(option => (
            <Box key={option.typeOfProduct} className={classes.requisiteElement}>
      ...
      {dealerServices.length > 0 && (
        <Box className={classes.requisiteContainer}>
          ...
          {dealerServices.map(option => (
            <Requisite key={option.typeOfProduct} additionalOption={option} />
          ))}
          <Divider />
        </Box>
      )}
      ...
      <SberTypography
        ...
        onClick={() => console.log('Изменить реквизиты')}
      >
        Изменить реквизиты
      </SberTypography>
    </Box>
