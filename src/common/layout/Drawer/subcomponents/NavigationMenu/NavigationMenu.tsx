...
import { Box, Button, ListItemButton, ListItemText, Tab, Tabs } from '@mui/material'
...
export function NavigationMenu(props: Props) {
  ...
  const renderLogout = useCallback((item: MenuItem) => (
    <Button
  ...
  const renderItem = useCallback((item: MenuItem, index: number) => {
  ...
  const value = Math.max(menuItems.findIndex((item) => matchPath(item.path, location.pathname)) , 0)
